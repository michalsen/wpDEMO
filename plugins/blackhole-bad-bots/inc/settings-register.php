...
function blackhole_callback_robots() {
	...
	if (($check === false) || (filter_var($site_url, FILTER_VALIDATE_URL) === false) || (filter_var($base_url, FILTER_VALIDATE_URL) === false)) {
		...
		$display  = '<em>'. esc_html__('Add the following rules to your site&rsquo;s robots.txt file:', 'blackhole-bad-bots') .'</em>';
...
function blackhole_callback_rate($args) {
	...
	$href  = 'https://wordpress.org/support/view/plugin-reviews/'. BBB_SLUG .'?rate=5#postform';
	$title = esc_attr__('Help keep Blackhole going strong! A huge THANK YOU for your support!', 'blackhole-bad-bots');
	
	echo '<a target="_blank" class="bbb-rate-plugin" href="'. $href .'" title="'. $title .'">'. $label .'</a>';
...
function blackhole_callback_pro($args) {
	...
	echo '<div class="blackhole-pro"><a target="_blank" href="'. $href .'" title="'. $label .'">';
