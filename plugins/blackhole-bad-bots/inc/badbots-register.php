...
function blackhole_callback_reset_badbots($args) {
	...
	echo '<a class="bbb-reset-badbots" href="'. $href .'">'. esc_html__('Reset Bad Bot Log', 'blackhole-bad-bots') .'</a>';
...
function blackhole_callback_blocked_bots() {
	...
	foreach ($bbb_badbots as $key => $val) {
		...
		$delete = ($ip_address === '123.456.789.000') ? '<span class="bbb-delete-bot-icon"></span>' : $delete_link;
